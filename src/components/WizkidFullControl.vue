<template >
  ...
  >
    ...
    <template v-slot:item.actions="{ item }">
      ...
      <v-icon small v-if="getIsAuth && item.is_fired==0" @click="fire(item)"> fire </v-icon>
      <v-icon small v-if="getIsAuth && item.is_fired==1" @click="unfire(item)"> unfire </v-icon>
    </template>
...
export default {
  ...
  methods: {
    ...mapActions(["fire","unfire","fetchWizkidsForUser", "addWizkid", "editWizkid","deleteWizkid","getIsAuth"]),
    initialize() {
      this.wizkids = [];
    },
    selectFile(event) {
